...
**必须向用户输出完整的 10 维度评估表格，每一维都带评分和说明。严禁跳过、折叠、或只在心里打分。** 表格是用户画像的核心交付物，不可省略。

基于 Phase 1 收集的 9 个答案，按 10 个维度给用户打分并解释：

1. **技能广度** — 跨领域数量 x 深度，编程/设计/销售/内容等各线覆盖
2. **人脉** — 过往生意/行业积累的人脉与潜在合作者
3. **行业/平台经验** — 是否做过小红书/闲鱼/B站/抖音/淘宝/公众号等
4. **工具熟练度** — Cursor/Claude Code/Codex/PS/剪映/PR/Canva/Figma/Excel/Python/前端
5. **时间弹性与持续性** — 每天可投入小时数及稳定性
6. **启动资金** — 从 0 到有多少可投入，0 启动天然加分
7. **可接单灵活性 / 能否露脸** — 是否愿意露脸出镜，影响内容/IP/直播类机会
8. **出海 / 多语能力** — 英语/其他外语水平及海外市场经验
9. **风险 / 沉没成本承受力** — 目标倾向（副业 vs 主业）、年龄阶段、家庭负担
10. **信息判断力** — 是否能识别卖课/割韭菜/用虚假截图宣传，做过生意+编程背景加分

### 输出格式（必须原样使用，不可改动）

Markdown 表格逐维呈现，每维必须包含评分（X/10）和一句话说明：
...
> 每个维度有 null 就正常说明，不打虚假高分。表格输出完毕后，紧跟一段"综合画像"定性总结（2-3句话），概括优势和短板。
## Phase 3 — 分层 + 写入
